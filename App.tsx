...
import { 
  ...
  Phone as PhoneIcon, MapPin, Upload, Download, Wallet, 
  History, Trash, CheckCircle2
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const generatePDF = async (shouldDownload: boolean = true) => {
    ...
    try {
      ...
      setStatusMessage({type: 'success', text: 'PDF Gerado com sucesso!'});
  ...
  const handleEmail = async () => {
    ...
    const bodyText = `Olá,\n\nSegue o recibo nº ${receiptNumber} referente a ${receiptType}.\n\nValor: R$ ${totalLiquido.toFixed(2)}\n\nAtenciosamente,\n${provider.name}`;
    if (navigator.share) {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-[#f8fafc]">
      ...
      <div className="lg:w-2/5 p-6 lg:p-8 no-print overflow-y-auto max-h-screen border-r border-slate-200 bg-white">
        <header className="mb-10 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-orange-600 text-[10px] font-black uppercase tracking-[0.3em] mt-1.5">Studio Gerador</p>
            </div>
          </div>
        ...
        {activeTab === 'form' ? (
          <div className="space-y-8 animate-in fade-in duration-500">
            <section ref={providerSectionRef} className="bg-slate-50/50 rounded-[32px] p-8 border border-slate-100 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                   {logo ? (
                     <div className="relative inline-block border-2 border-white bg-white p-3 rounded-2xl shadow-sm group">
                       ...
                       <button onClick={() => setLogo(null)} className="absolute -top-3 -right-3 bg-rose-500 text-white p-1 rounded-full shadow-lg"><X size={12}/></button>
                     </div>
                   ) : (
                     <label className="h-24 w-full border-2 border-dashed border-slate-200 rounded-[24px] flex flex-col items-center justify-center cursor-pointer hover:bg-white text-slate-400 transition-all">
                       <Upload size={24} />
                       <span className="text-[9px] font-black mt-2 uppercase">Logo Emissor</span>
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" className={getFieldClass('providerName', "w-full px-5 py-3.5 bg-white border rounded-2xl font-bold uppercase text-[13px] outline-none shadow-sm")} value={provider.name} onChange={e => setProvider({...provider, name: e.target.value})} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase text-slate-400">CPF ou CNPJ *</label>
                  <input type="text" className={getFieldClass('providerDoc', "w-full px-5 py-3.5 bg-white border rounded-2xl font-bold text-[13px] outline-none shadow-sm")} value={provider.document} onChange={e => setProvider({...provider, document: maskDocument(e.target.value)})} />
                </div>
                <div className="md:col-span-2 space-y-1.5">
                  <label className="text-[10px] font-black uppercase text-slate-400">Dados Bancários / PIX *</label>
                  <textarea className={getFieldClass('bankInfo', "w-full px-5 py-4 bg-white border rounded-2xl font-bold uppercase text-[13px] h-20 resize-none outline-none shadow-sm")} value={provider.bankInfo} onChange={e => setProvider({...provider, bankInfo: e.target.value})} />
                </div>
                <div className="md:col-span-2 space-y-4">
                  ...
                  <div className="h-32 bg-white rounded-[24px] border-2 border-slate-100 relative cursor-crosshair overflow-hidden touch-none shadow-inner">
            ...
            <section ref={itemsSectionRef} className="bg-slate-50/50 rounded-[32px] p-8 border border-slate-100 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3 text-orange-600"><Briefcase size={18}/><h3 className="font-black uppercase text-[11px] tracking-[0.2em]">Itens do Recibo</h3></div>
                <div className="flex bg-white p-1 rounded-2xl border border-slate-100 shadow-sm">
                   {Object.values(ReceiptType).map(t => (
                     <button key={t} onClick={() => setReceiptType(t)} className={`px-4 py-2 text-[9px] font-black uppercase rounded-xl transition-all ${receiptType === t ? 'bg-orange-600 text-white' : 'text-slate-400'}`}>
              ...
              <div className="space-y-5">
                {items.map((item, idx) => (
                  <div key={item.id} className="p-6 bg-white rounded-[24px] border border-slate-100/50 space-y-4 shadow-sm">
                    <div className="flex gap-4">
                      ...
                      {items.length > 1 && <button onClick={() => setItems(items.filter(i => i.id !== item.id))} className="mt-8 text-rose-500"><Trash2 size={20}/></button>}
                    </div>
                  </div>
                ))}
                <button onClick={() => setItems([...items, {id: Math.random().toString(), description:'', value:0}])} className="w-full py-4 border-2 border-dashed border-orange-200 rounded-[24px] text-orange-600 font-black uppercase text-[10px] flex items-center justify-center gap-3 hover:bg-orange-50 transition-all"><Plus size={18}/> Adicionar Item</button>
              </div>
            </section>

            <div className="grid grid-cols-2 gap-6 sticky bottom-6">
              <button onClick={() => generatePDF(true)} disabled={isPrinting} className="py-5 bg-orange-600 text-white rounded-[24px] font-black uppercase text-[11px] shadow-2xl flex items-center justify-center gap-3 disabled:opacity-50 active:scale-95 transition-transform">
                {isPrinting ? <Loader2 size={22} className="animate-spin"/> : <Printer size={22}/>} Baixar PDF
              </button>
              <button onClick={handleEmail} disabled={isPrinting} className="py-5 bg-slate-900 text-white rounded-[24px] font-black uppercase text-[11px] flex items-center justify-center gap-3 active:scale-95 transition-transform">
          ...
          <div className="space-y-5 animate-in slide-in-from-right duration-500">
            {history.length === 0 ? <div className="text-center py-24 text-slate-200 uppercase font-black text-[10px] tracking-widest">Histórico Vazio</div> : history.map(entry => (
              <div key={entry.id} className="bg-slate-50 p-6 rounded-[28px] flex items-center justify-between border border-slate-100 shadow-sm">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={() => setHistory(prev => prev.filter(h => h.id !== entry.id))} className="p-3 text-rose-500 hover:bg-rose-50 rounded-xl transition-colors"><Trash size={16}/></button>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* VISUALIZAÇÃO DO RECIBO (A4) */}
      <div className="lg:w-3/5 bg-slate-200/50 p-6 lg:p-12 flex justify-center items-start overflow-y-auto h-screen no-print no-scrollbar">
        <div className="bg-white shadow-2xl relative scale-[0.6] md:scale-[0.8] lg:scale-[1] origin-top flex flex-col" id="printable-receipt" style={{ width: '210mm', minHeight: '297mm', padding: '12mm 15mm', boxSizing: 'border-box' }}>
          <div className="flex justify-between items-center mb-6 pb-6 border-b-2 border-slate-100">
            <div className="flex items-center gap-6">
              {logo ? <img src={logo} className="h-14 w-auto object-contain" /> : <div className="h-12 w-12 bg-slate-50 border rounded-xl flex items-center justify-center text-slate-200"><ImageIcon size={20}/></div>}
              <div className="border-l-4 border-slate-100 pl-6">
                <h1 className="text-[28px] font-black tracking-tighter uppercase italic leading-none">RODAMOINHO</h1>
                <p className="text-[9px] font-black text-orange-600 uppercase tracking-[0.5em] mt-2">PRODUÇÕES ARTÍSTICAS</p>
              </div>
            </div>
            <div className="text-right">
              <div className="bg-orange-600 text-white px-5 py-2 rounded-full text-[10px] font-black uppercase mb-2 shadow-md">DOC Nº {receiptNumber}</div>
          ...
          <div className="grid grid-cols-2 gap-10 mb-8">
             <div className="space-y-6">
                <div className="p-6 bg-slate-50/50 rounded-[24px] space-y-1.5 text-[10px] border border-slate-100/50">
                   <p className="text-orange-600 font-black mb-2 uppercase tracking-widest">TOMADOR</p>
                   <p className="font-black text-[11px] mb-0.5">{CLIENT_DATA.name}</p>
                   <p className="text-slate-500 font-bold">CNPJ: {CLIENT_DATA.cnpj}</p>
                   <p className="text-slate-400 font-bold uppercase">{CLIENT_DATA.address}<br/>{CLIENT_DATA.neighborhood}<br/>CEP: {CLIENT_DATA.cep} • I.M: {CLIENT_DATA.im}</p>
                </div>
                <div className="p-6 bg-white border-2 border-slate-50 rounded-[24px] space-y-1.5 text-[10px]">
                   <p className="text-orange-600 font-black mb-2 uppercase tracking-widest">EMISSOR</p>
                   <p className="font-black text-[11px] mb-0.5 uppercase">{provider.name || "---"}</p>
                   <p className="text-slate-500 font-bold">DOC: {provider.document || "---"}</p>
                   <p className="text-slate-400 font-bold uppercase mt-2 italic flex items-center gap-2"><MapPin size={10}/> {city.toUpperCase()}</p>
                </div>
             </div>
             <div className="flex flex-col justify-between">
                <div className="bg-[#1a1f2c] rounded-[36px] p-8 flex flex-col items-center justify-center text-center shadow-xl min-h-[140px]">
                   <Wallet className="text-orange-600 mb-2" size={28}/>
                   <p className="text-orange-600 text-[9px] font-black uppercase tracking-[0.4em] mb-2">VALOR LÍQUIDO</p>
                   <p className="text-white text-[34px] font-black tracking-tighter">R$ {totalLiquido.toFixed(2)}</p>
                </div>
                <div className="mt-6 bg-orange-50/50 p-6 rounded-[28px] text-center border-2 border-orange-100/30">
                   <h4 className="text-[9px] font-black text-orange-600 uppercase mb-3 tracking-[0.3em]">CREDITAR EM:</h4>
                   <p className="text-[12px] font-black text-[#1a1f2c] uppercase italic leading-tight">{provider.bankInfo || "---"}</p>
                </div>
             </div>
          </div>
          ...
          <table className="w-full flex-grow">
            <thead>
               <tr className="bg-slate-50/80 border-y-2 border-slate-100">
                  <th className="text-left py-4 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest italic">DESCRIÇÃO DOS ITENS</th>
                  <th className="text-right py-4 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest italic w-40">VALOR (R$)</th>
               </tr>
            </thead>
            <tbody className="divide-y divide-slate-100/60">
               {items.map((item, i) => (
                 <tr key={item.id}>
                    <td className="py-5 px-6 font-black text-[13px] uppercase italic text-[#1a1f2c]">{i+1}. {item.description || "---"}</td>
                    <td className="py-5 px-6 text-right font-black text-[#1a1f2c] text-[14px]">{item.value.toFixed(2)}</td>
                 </tr>
               ))}
            </tbody>
          </table>
          ...
          <div className="pt-8 border-t-4 border-[#1a1f2c] flex flex-col items-center">
             <div className="h-28 flex items-end mb-2">{provider.signature && <img src={provider.signature} className="h-24 w-auto object-contain" />}</div>
             <div className="w-72 h-[1mm] bg-[#1a1f2c] mb-4"></div>
             <p className="font-black text-[14px] uppercase tracking-tighter text-[#1a1f2c]">{provider.name || "NOME DO EMISSOR"}</p>
             ...
             <div className="mt-6 flex items-center gap-3 bg-slate-50 px-5 py-2 rounded-full border border-slate-200">
                ...
                <span className="text-[7px] font-black text-slate-400 uppercase tracking-[0.3em]">VERIFICAÇÃO DIGITAL ATIVA</span>
             </div>
          </div>
          <footer className="mt-8 text-center border-t border-slate-50 pt-6 opacity-20"><p className="text-[8px] font-black uppercase tracking-[1em]">RODAMOINHO • PRODUÇÃO ARTÍSTICA</p></footer>
        </div>
      </div>
    </div>
  );
